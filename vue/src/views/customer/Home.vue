          ...
          <div v-else>
            <div v-if="shops.length" class="grid grid-cols-3" >
              ...
              'my-1 text-sm font-bold px-2 py-1 rounded-md border-2 text-green-500 border-green-500']">
                    ...
                    <router-link :to="shop.status === 'open' ? {name : 'CustomerShopView', params : { id : shop.id}} : ''" :class="[shop.status === 'open' ? 'inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-gray-800 rounded-lg hover:bg-gray-900' : 'inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-gray-500 rounded-lg cursor-not-allowed']">
                        View
